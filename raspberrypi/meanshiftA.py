...
def onMouse(event, x,y,flags,param):
    ...
    if inputmode:
        ...
        elif event == cv2.EVNET_MOUSEMOVE:
            if rectangle:
                frame=frame2.copy()
                cv2.rectangle(frame,(col,row),(x,y),(0,255,0),2)
                cv2.imshow('frame',frame)
                
        elif evnet==cv2.EVENT_LBUTTONUP:
            inputmode=False
            rectangle=False
            cv2.rectagle(frame,(col,row),(x,y),(0,255,0),2)
            height, width=abs(row-y), abs(col-x)
            trackWindow=(col,row,width,height)
            roi=frame[row:row+height, col:col_width]
            roi=cv2.cvtColor(roi,cv2,COLOR_BGR2HSV)
            roi_hist=cv2.calcHist([roi],[0],None,[180],[0, 180])
            cv2.normalize(roi_his, roi_hist ,0, 255,cv2.NORM_MINMAX)
    return
